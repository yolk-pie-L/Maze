...
if x.tag==0
    ...
    uicontrol('Parent',x.f2,'Style','pushbutton','Units','normalize','String','返回菜单','FontName','幼圆','Fontsize',12,'Position',[0.01 0.8 0.1 0.08],...
        'Callback','global x;delete([x.f1,x.f2]); clear sound; MazeInterface;','enable','off');
    uicontrol('Parent',x.f2,'Style','pushbutton','Units','normalize','String','暂停游戏','FontName','幼圆','Fontsize',12,'Position',[0.01 0.7 0.1 0.08],...
        'Callback','global x;x.tag=-1.1;x.step=1;MazeInterface(x);','tag','wait','enable','off');
    uicontrol('Parent',x.f2,'Style','pushbutton','Units','normalize','String','重新开始','FontName','幼圆','Fontsize',12,'Position',[0.01 0.6 0.1 0.08],...
        'Callback','global x;x.tag=-1;x.passtime=0;x.step=1;x.log=[];x.pos=[2,1];MazeInterface(x);','enable','off');
    uicontrol('Parent',x.f2,'Style','pushbutton','Units','normalize','String','更新地图','FontName','幼圆','Fontsize',12,'Position',[0.01 0.5 0.1 0.08],...
        'Callback','global x;x.tag=-2;x.passtime=0;x.step=1;x.log=[];x.pos=[2,1];MazeInterface(x);','enable','off');
    uicontrol('Parent',x.f2,'Style','pushbutton','Units','normalize','String','求解地图','FontName','幼圆','Fontsize',12,'Position',[0.01 0.4 0.1 0.08],...
        'Callback','global x;x.step=1.3;x.tag=1;MazeInterface(x);','enable','off');
    uicontrol('Parent',x.f2,'Style','pushbutton','Units','normalize','String','通关回放','FontName','幼圆','Fontsize',12,'Position',[0.01 0.3 0.1 0.08],...
        'Callback','global x;x.step=1.4;MazeInterface(x);','enable','off');
    uicontrol('Parent',x.f2,'Style','pushbutton','Units','normalize','String','输出迷宫','FontName','幼圆','Fontsize',12,'Position',[0.01 0.2 0.1 0.08],...
